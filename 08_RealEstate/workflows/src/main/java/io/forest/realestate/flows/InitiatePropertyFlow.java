...
public class InitiatePropertyFlow extends FlowLogic<SignedTransaction>{
	...
	public SignedTransaction call() throws FlowException {
		...
		TransactionBuilder builder = new TransactionBuilder(notary).addOutputState(propertyDetails, PropertyContract.ID)
				.addCommand(new PropertyCommands.Create(), getOurIdentity().getOwningKey())
				.setTimeWindow(Instant.now(), Duration.ofSeconds(10));
		// We are adding a time window of 10 seconds for the Notary service to sign the
		// transaction replacing default one
